-- A slightly modified version of Civ 5 vanilla's MainMenu
  ...
  if(type == SystemUpdateUIType.RestoreUI) then
		if (tag == "MainMenu") then
			if (registered == false) then
				...
				Events.FrontEndPopup.Add(activateMods)
				print("Registered the activate mods callback!")
				registered = true
			end
	...
	elseif (type == SystemUpdateUIType.ReloadUI) then
		if (registered == false) then
			...
			Events.FrontEndPopup.Add(loadSaves)
			registered = true
		end
	end
end
