...
**Implemented:** telepathist-prep.ts wraps existing `prepareTurnSummaries` for batch use. extractor.ts provides `extractTurnContexts()` (batch queries all game DB tables) and `extractPlayerEpisodes()` (per-player turn iteration building RawEpisode[]). index.ts wires both into the game processing loop with `--game`/`-g` filter (not `--experiment`). CLI entry point: `npm run archivist -- -a <archive> -o <output.duckdb> [-g gameId] [--force]`.
