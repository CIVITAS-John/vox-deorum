...
:: Banner
echo.
echo ============================================echo       Vox Deorum Quick Start Script        echo ============================================echo.
...
:: Check for Git
echo [1/4] Checking for Git...where git >nul 2>&1
if %errorlevel% equ 0 (
    echo   [OK] Git is already installed    for /f "delims=" %%i in ('git --version') do echo     %%i
    goto :CloneRepo
)
...
if %errorlevel% equ 0 (
    ...
    if !errorlevel! equ 0 (
        echo   [OK] Git installed via winget        goto :RefreshPath
    )
)
...
if not exist "%GIT_INSTALLER%" (
    echo   [FAIL] Failed to download Git installer    echo.
...
if %errorlevel% neq 0 (
    echo   [WARN] Git installation may have failed    echo   If Git is not installed, please install manually and run this script again.
    pause
)
...
where git >nul 2>&1
if %errorlevel% neq 0 (
    echo   [FAIL] Git still not found in PATH    echo   You may need to:
...
if exist "%INSTALL_DIR%" (
    echo   [WARN] Installation directory already exists: %INSTALL_DIR%    choice /C YN /M "Delete existing directory and reinstall?"
    if errorlevel 2 (
        ...
        if %errorlevel% equ 0 (
            ...
            echo   [FAIL] Directory exists but is not a Git repository            echo   Please delete or rename: %INSTALL_DIR%
            pause
            exit /b 1
        )
        goto :InitSubmodules
...
if %errorlevel% neq 0 (
    echo   [FAIL] Failed to clone repository    echo.
    echo   Please check:
...
echo   [OK] Repository cloned successfullycd /d "%INSTALL_DIR%"
...
if %errorlevel% neq 0 (
    echo   [WARN] Some submodules may have failed to download    echo   You can manually update them later with:
...
if not exist "%INSTALL_SCRIPT%" (
    echo   [FAIL] Installation script not found at:    echo   %INSTALL_SCRIPT%
    echo.
...
echo.
echo ============================================echo    Starting Vox Deorum Installation...     echo ============================================echo.
...
if %errorlevel% equ 0 (
    echo.
    echo ============================================    echo       Quick Start Completed Successfully!       echo ============================================    echo.
    ...
    echo.
    echo ============================================    echo     Quick Start Completed with Warnings        echo ============================================    echo.
